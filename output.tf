output "core_network_rg_id" {
  ...
  value       = var.create_rg ? tostring(azurerm_resource_group.compute_networking_rg[0].id) : ""
...
output "core_vnet_id" {
  ...
  value       = var.create_vnet ? tostring(azurerm_virtual_network.compute_vnet[0].id) : ""
